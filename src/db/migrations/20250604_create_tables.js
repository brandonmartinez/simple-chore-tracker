export async function up(knex) {
  ...
  await knex.schema.createTable('ChoreAvailableAssignments', (table) => {
    table.increments('id').primary();
    table.integer('chore_id').unsigned().references('id').inTable('Chores').onDelete('CASCADE');
  ...
  await knex.schema.createTable('ChoreAssignments', (table) => {
    ...
    table.integer('chore_id').unsigned().references('id').inTable('Chores').onDelete('CASCADE');
    table.integer('person_id').unsigned().references('id').inTable('People').onDelete('CASCADE');
    table.integer('time_period_id').unsigned().references('id').inTable('TimePeriods').onDelete('CASCADE');
    table.timestamps(true, true);
  });
